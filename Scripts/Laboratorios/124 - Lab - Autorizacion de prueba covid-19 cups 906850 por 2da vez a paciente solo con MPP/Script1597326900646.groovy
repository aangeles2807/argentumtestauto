...
for(int i=1; i <= 2; i++){
	...
	if (i == 1) {
		...
		mapaVariablesScript = WS.callTestCase(findTestCase('Comun/ProcesoAutorizacion'), [
			...
			'fechaAutorizacion' : Keyword.AFILIADO_AUTORIZACION_SYSDATE.value,
			'servicioConsulta' : Keyword.SERVICIO_LABORATORIO.value,
			'condicionProcedimiento': Keyword.PROCEDIMIENTO_CODIGO.value + "'906850'"], FailureHandling.STOP_ON_FAILURE);
	}
	else if (i == 2) {
